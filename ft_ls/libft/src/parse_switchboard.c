...
static char		double_checker(char *fmt, char c)
{
	...
	result = 0;
	while (ft_fstrchr("+-#0 .123456789hljz", *fmt))
	{
		if (*fmt == c)
			result++;
		fmt++;
...
int				parse_switchboard(char **fmt, va_list arg)
{
	...
	struct_start(&tab);
	while (ft_fstrchr(CHECK_LIST, **fmt))
	{
		(ft_fstrchr(FLAGS, **fmt)) ? parse_flags(fmt, &tab) : 0;
		if (ft_fstrchr("123456789", **fmt))
		{
			tab.f_width = ft_fatoi(fmt);
			continue;
		}
		if (**fmt == '.' && (*fmt)++)
		{
			tab.precision = ft_fatoi(fmt);
			continue ;
		}
		(ft_fstrchr(LEN_MOD, **fmt)) ? parse_modifier(fmt, &tab) : 0;
		(*fmt)++;
	}
	if (ft_fstrchr(SPECIFIERS, **fmt))
		tab.specifier = **fmt;
	else
		tab.specifier = **fmt;
